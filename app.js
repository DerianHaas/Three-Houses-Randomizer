$(() => {
    let result;
    let route;
    let avoidCanonClasses;
    $("#submit").click(() => {
        route = parseInt($("#routeSelect").val());
        ...
        $("#result").append($("<table class='resultTable'>").append($("<tr>").append("<th>Name</th><th>Class</th><th>Proficiency Score</th>"), result.map(res => {
            ...
            if (profScore <= 0.5)
                scoreClass = "badScore";
            return $("<tr>").append($("<td class='nameCell'>").append(res.char.name), $("<td class='classCell'>").append(res.class.name), $("<td class='scoreCell " + scoreClass + "'>").append("" + profScore), $("<td class='buttonCell'>").append(res.char.required ? "" : "<button class='replaceChar'>Reroll Character</button><br/><br/>", "<button class='replaceClass'>Reroll Class</button>"));
        })));
    });
